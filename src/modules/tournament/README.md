...
| `status`       | Текущее состояние игрока, например `ACTIVE` или `FINISHED`. |
...
- если `status` переводится в `FINISHED`, а `bustoutOrder` еще равен `0`,
  backend автоматически ставит следующий порядок вылета внутри события: `max(bustoutOrder) + 1`.
  Переданный вручную `bustoutOrder > 0` сохраняется.
